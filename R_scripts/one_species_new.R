...
p2 <- ggplot(dfsave.tot[dfsave.tot$years > 20,], aes(x = years, y = SSBmean))+
...
p3 <- ggplot(df.save, aes(x = SSB,  y = Rscam, color = factor(run)))+geom_line()+geom_line()+facet_grid(rho ~ model)+
  theme_classic()+geom_point(aes(y = R), alpha = 0.05)+#geom_line(aes(y = SR), col = 'black')+
  theme(legend.position = 'none')
p3

ggsave(p3, filename = 'figures/SR_relations.png', width = 20, height = 16, units = 'cm')

## Look at some of the lambda runs individually 

p1 <-  ggplot(prop.plot[prop.plot$lambda == 0.5 & prop.plot$model %in% c('linear-BOFF','linear-noBOFF'),], 
              aes(x = model, y = value, fill = model))+geom_violin()+
  geom_boxplot(width = 0.1)+
  facet_grid(factor(rho)~name)+scale_x_discrete()+
  theme_bw()+theme(axis.text.x = element_blank())+coord_cartesian(ylim = c(-lims,lims))+geom_hline(aes(yintercept = 0), linetype = 2)
p1

p2 <-  ggplot(prop.plot[prop.plot$rho == 0.9 & prop.plot$model %in% c('linear-BOFF','linear-noBOFF'),], aes(x = model, y = value, fill = model))+geom_violin()+geom_boxplot(width = 0.1)+
  facet_grid(factor(lambda)~name)+scale_x_discrete()+
  theme_bw()+theme(axis.text.x = element_blank())+coord_cartesian(ylim = c(-lims,lims))+geom_hline(aes(yintercept = 0), linetype = 2)
p2


ggsave('figures/rho_0.1_violin.png', p1, width = 16, height = 16)
ggsave('figures/rho_0.9_violin.png', p2, width = 16, height = 16)
...
dp <- df.save[df.save$rho == 0.1 & df.save$lambda %in% c(0,.5) & df.save$years > 10,]

ggplot(dp, aes(x = SSB, y = R, color = model))+geom_point(alpha = .1)+theme_bw()
ggplot(dp, aes(x = SSB, y = Rtot, color = model))+geom_point(alpha = .05)+theme_bw()

ggplot(dp, aes(x = xfrac, y = R/SSB, color = model))+geom_point(alpha = .05)+theme_bw()+geom_smooth()
